...
- [ ] IntelliJ Run Configuration 환경변수 설정 방법
...
- [ ] 외부 설정 주입 방식 이해 (환경변수, ${...} placeholder)
- [ ] Spring Boot 실행 환경과 IntelliJ Run Configuration의 차이 이해
...
- [ ] 공통 응답 형식 설계 (`ApiResponse<T>`)
- [ ] 전역 예외 처리 (@ControllerAdvice, @ExceptionHandler)
- [ ] 예외 처리 구조 설계 (ErrorCode, BusinessException, GlobalExceptionHandler)
...
- [ ] JPA Auditing 설정 (@CreatedDate, @LastModifiedDate, @EnableJpaAuditing)
- [ ] 지연로딩 vs 즉시로딩
...
- [ ] 패키지 구조 설계 기준 (도메인별 분리 + 계층별 분리 + global 공통 영역)
- [ ] DTO / VO / Entity 분리 원칙
- [ ] REST API URL 설계 규칙
- [ ] 빌더 패턴 (@Builder 활용)
- [ ] 디자인 패턴: 싱글톤
