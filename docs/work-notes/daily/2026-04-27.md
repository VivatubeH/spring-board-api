...
- `application.yml`은 들여쓰기로 계층 구조를 표현하는 방식으로 작성합니다.
...
- `application` : 애플리케이션 이름과 같은 기본 정보를 설정한다.
...
- 이 때 데이터베이스 주소, 사용자명, 비밀번호와 같은 접속 정보가 필요하며, 이러한 정보를 `application.yml`의 `datasource` 설정에서 관리한다.
...
### JPA란?
- JPA(Java Persistence API)는 자바 객체와 데이터베이스 테이블을 매핑하여 데이터를 저장하고 조회할 수 있도록 도와주는 기술이다.
- SQL을 직접 작성하는 방식보다 객체 중심으로 데이터를 다룰 수 있게 해주며, Spring Boot에서는 주로 Hibernate 구현체와 함께 사용한다.

### JPA가 필요한 이유
- 데이터를 자바 객체 중심으로 다루면서, 데이터베이스에 저장하고 조회할 수 있도록 하기 위해서 필요하다.
- SQL을 반복 작성하는 부담을 줄이고, 객체와 테이블의 매핑을 더 효율적으로 관리할 수 있다.

### application.yml에서 정리할 JPA 설정 항목
- `hibernate.ddl-auto` : 애플리케이션 실행 시 테이블을 어떻게 생성하거나 반영할지 설정한다.
- `show-sql` : 실행되는 SQL을 콘솔에 출력할지 설정한다.
- `properties.hibernate.format_sql` : 출력되는 SQL을 보기 좋게 정렬할지 설정한다.

### JPA 기본 설정 반영
- 개발 단계에서 테이블 반영 방식과 실행되는 SQL을 확인하기 위해 JPA 기본 설정을 추가한다.
- 우선 `ddl-auto`, `show-sql`, `format_sql`과 같은 기본 항목을 반영하여, 이후 엔티티 설계와 실행 과정에서 동작을 확인할 수 있도록 구성한다.
  ...
  jpa:
    ...
    properties:
      hibernate:
        format_sql: true
```
- `ddl-auto` : `update` - 애플리케이션 실행 시 변경된 엔티티 구조를 테이블에 반영한다.
- `show-sql` : `true` - 실행되는 SQL을 콘솔에 출력한다.
- `format_sql` : `true` - 출력되는 SQL을 보기 좋게 정렬한다.
...
### Auditing이란?
- Auditing은 데이터의 생성 시각과 수정 시각 같은 변경 이력을 자동으로 관리하는 기능이다.
- JPA Auditing을 사용하면 엔티티의 공통 이력 값을 자동으로 기록할 수 있다.

### Auditing이 필요한 이유
- 생성 시각과 수정 시각 같은 공통 이력 값을 직접 관리하지 않고 자동으로 처리하기 위해 Auditing이 필요하다.
- 이를 통해 중복 코드를 줄이고, 일관된 방식으로 이력 정보를 관리할 수 있다.

### Auditing으로 자동 관리할 값
- Auditing을 통해 엔티티의 생성 시각과 마지막 수정 시각을 자동으로 관리할 수 있다.
- 이러한 공통 이력 값은 여러 엔티티에서 함께 사용할 수 있다.

### Auditing 설정 방향
- 생성 시각과 수정 시각은 여러 엔티티에서 공통으로 필요한 값이므로, 한 곳에서 함께 관리하는 방향으로 설계하고자 했다.
- 이후 JPA Auditing을 적용하여 엔티티가 저장되거나 수정될 때 해당 값이 자동으로 기록되도록 구성할 예정이다.
